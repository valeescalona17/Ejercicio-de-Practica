...
export default class Cl_mVenta extends Cl_mTablaWeb {
  ...
  get ventaOk(): string | true {
    ...
    if (!this.cantidadJuegosOk) return "La cantidad de juegos es inválida";
    if (!this.cantidadPelucheOk) return "La cantidad de peluches es inválida";
    if (!this.cantidadMesaOk) return "La cantidad de juegos de mesa es inválida";
    return true;
  }
  ...
  cantidadProductosPorTipo(ventas: Cl_mVenta[]): {
    cantJuegos: number;
    cantMesa: number;
    cantPeluche: number;
  } {
  ...
  informacionVentasProducto({
    ...
    const filtrarCodigo = ventas.filter((v) => v.codigo === codigo);
    const resultados: iInfoAsistente[] = [];
    const producto = catalogo.productos.find((p) => p.codigo === codigo);
    ...
    for (const v of filtrarCodigo) {
      const unidsCompradas =
        v.cantidadJuegos + v.cantidadMesa + v.cantidadPeluche;
